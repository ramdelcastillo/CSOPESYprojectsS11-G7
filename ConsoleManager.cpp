...
void ConsoleManager::createProcessConsole(String screenName)
{
    ...
    std::shared_ptr<AttachedProcess> process = std::make_shared<AttachedProcess>(this->madePid, screenName, getConfigParser()->getMemoryPerFrame(), getConfigParser()->getMinMemoryPerProcess(), getConfigParser()->getMaxMemoryPerProcess());
...
void ConsoleManager::setupScheduler(uint32_t nCores, std::string currentSchedulingAlgo, uint32_t quantumCycles, uint32_t batchProcessFreq, uint32_t minIns, uint32_t maxIns, uint32_t delaysPerExecution, uint32_t maxOverallMemory, uint32_t memoryPerFrame, uint32_t minMemoryPerProcess, uint32_t maxMemoryPerProcess)
{
    
    cpuScheduler = std::make_shared<CPUScheduler>(nCores, currentSchedulingAlgo, quantumCycles, batchProcessFreq, minIns, maxIns, delaysPerExecution, maxOverallMemory, memoryPerFrame, minMemoryPerProcess, maxMemoryPerProcess);
}

void ConsoleManager::setupBackingStore()
{
    backingStore = std::make_shared<BackingStore>("backing-store.txt", "backing-store-log.txt");
}

void ConsoleManager::setupMemoryAllocator()
{
    if (getConfigParser()->getMaxOverallMemory() == getConfigParser()->getMemoryPerFrame()) {
        this->memoryAllocator = std::make_shared<FlatMemoryAllocator>();
    }
    else {
        this->memoryAllocator = std::make_shared<PagingAllocator>();
    }
}
...
std::shared_ptr<BackingStore> ConsoleManager::getBackingStore()
{
    return backingStore;
}
...
void ConsoleManager::removeFromConsoleTable(std::string screenName)
{
    auto it = consoleTable.find(screenName);
    if (it != consoleTable.end()) {
        consoleTable.erase(it);
        std::cout << "Removed process console for screen: " << screenName << std::endl;
    }
}
