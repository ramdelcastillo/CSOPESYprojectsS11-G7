...
CPUScheduler::CPUScheduler(uint32_t nCores, std::string currentSchedulingAlgo, uint32_t quantumCycles, uint32_t batchProcessFreq, uint32_t minIns, uint32_t maxIns, uint32_t delaysPerExecution)
    : nCores(nCores), currentSchedulingAlgo(currentSchedulingAlgo), quantumCycles(quantumCycles), batchProcessFreq(batchProcessFreq), minIns(minIns), maxIns(maxIns), delaysPerExecution(delaysPerExecution), nAmountOfWorkingCores(0)
{
...
void CPUScheduler::batchProcessLoop() {
    ...
    while (scheduling.load()) {
        ...
        if (currentCycle % batchProcessFreq == 0 && currentCycle != lastCycleProcessed) {
            ...
            std::shared_ptr<AttachedProcess> process = std::make_shared<AttachedProcess>(processId, processName);
...
void CPUScheduler::updateAmountOfWorkingCores()
{
    std::lock_guard<std::mutex> lock(schedulerMutex);
    ...
    nAmountOfWorkingCores = busyCores;  
}

void CPUScheduler::updateCores()
{
    while (true) {
        updateAmountOfWorkingCores();
    }
}
