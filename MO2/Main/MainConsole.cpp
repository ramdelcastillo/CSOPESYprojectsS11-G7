...
void MainConsole::processCommand(String command) {
    ...
    if (!consoleManager->getIsFunctionalityInitialized()) {
        if (command == "initialize" || command == "in") {
            ...
            uint32_t minMemoryPerProcess = consoleManager->getConfigParser()->getMinMemoryPerProcess();
            uint32_t maxMemoryPerProcess = consoleManager->getConfigParser()->getMaxMemoryPerProcess();
            consoleManager->setupScheduler(nCpu, schedulingAlgo, quantumCycles, batchProcessFreq, minIns, maxIns, delaysPerExecution, 
                maxOverallMemory, memoryPerFrame, minMemoryPerProcess, maxMemoryPerProcess);
            consoleManager->setupMemoryAllocator();
            consoleManager->setupBackingStore();
            clearScreen();
            header();
        ...
        if (tokens.size() == 0) {
            ...
            else if (command == "d" || command == "process-smi") {
                consoleManager->getMemoryAllocator()->visualizeMemory();
            }
            else if (command == "f" || command == "vmstat") {
