...
public:
	...
	void setupScheduler(uint32_t nCores, std::string currentSchedulingAlgo, uint32_t quantumCycles, uint32_t batchProcessFreq, uint32_t minIns, uint32_t maxIns, uint32_t delaysPerExecution, uint32_t maxOverallMemory, uint32_t memoryPerFrame, uint32_t minMemoryPerProcess, uint32_t maxMemoryPerProcess);
	void setupBackingStore();
	void setupMemoryAllocator();
	std::shared_ptr<CPUScheduler> getCPUScheduler();
	std::shared_ptr<IMemoryAllocator> getMemoryAllocator();
	std::shared_ptr<BackingStore> getBackingStore();
	int getMadePid();
	void addOneToMadePid();
	void addToConsoleTable(std::string screenName, std::shared_ptr<ProcessConsole> newConsole);
	void removeFromConsoleTable(std::string screenName);
